...
### 2.6 3D Spatial Spherical Graph & Blast Radius Engine
Transforms raw AST and import graphs into an interactive 3D spatial spherical visualization (`BlastRadiusCanvas`):
- **Perspective 3D Projection**: Simulates camera distance ($800\text{px}$), pitch ($\theta_x$), yaw ($\theta_y$), and scale zoom.
- **Z-Buffer Depth Sorting**: Orders nodes from furthest to closest with realistic alpha attenuation and depth scaling.
- **Dynamic Blast Radius Telemetry**: Evaluates coupling risk % (0%–100%) and cascading dependency radius when modifying any file or symbol.
- **Interactive Touch Inspection**: Direct hit-testing on nodes opens a bottom sheet modal detailing affected components and suggested safe refactoring strategies.
